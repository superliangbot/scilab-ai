...
const PLACEHOLDER_SLUG = "heart-electrical-system";

const PlaceholderFactory: SimulationFactory = () => {
  const config = getSimConfig(PLACEHOLDER_SLUG.replace("PLACEHOLDER", "heart-electrical-system"))!;
  ...
  const engine: SimulationEngine = {
    ...
    render() {
      ...
      ctx.fillText("heart-electrical-system Simulation", W/2, H/2);
    },
    reset() { time = 0; },
    destroy() {},
    getStateDescription() { return "heart-electrical-system simulation placeholder."; },
    resize(w, h) { W = w; H = h; }
  };
  return engine;
};
