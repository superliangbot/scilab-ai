...
const PLACEHOLDER_SLUG = "genetics-punnett-square";

const PlaceholderFactory: SimulationFactory = () => {
  const config = getSimConfig(PLACEHOLDER_SLUG.replace("PLACEHOLDER", "genetics-punnett-square"))!;
  ...
  const engine: SimulationEngine = {
    ...
    render() {
      ...
      ctx.fillText("genetics-punnett-square Simulation", W/2, H/2);
    },
    reset() { time = 0; },
    destroy() {},
    getStateDescription() { return "genetics-punnett-square simulation placeholder."; },
    resize(w, h) { W = w; H = h; }
  };
  return engine;
};
